...
VarDec:  ID {$$=new NVarDec(*(new NIdentifier($1))); $$->line=yylineno;}   //ID结点，标识符符号串存放结点的type_id
        | VarDec LB INT RB {$$->line=$1->line;}
        ;
FunDec: ID LP VarList RP {$$=new NFunDec(*(new NIdentifier($1)),$3); $$->line=$3->line;}
...
Stmt:     Exp SEMI {$$=new NExpStmt(*$1); $$->line=$1->line;}
        ...
        | IF LP Exp RP Stmt %prec LOWER_THEN_ELSE   {$$=new NIfStmt(*$3,*$5); $$->line=$3->line;}
        | IF LP Exp RP Stmt ELSE Stmt {$$=new NIfElseStmt(*$3,*$5,*$7); $$->line=$3->line;}
        | WHILE LP Exp RP Stmt {$$=new NWhileStmt(*$3,*$5); $$->line=$3->line;}
        | BREAK SEMI {$$=new NBreakStmt(); $$=nullptr;}
        ;
...
Exp:      Exp ASSIGNOP Exp {$$=new NAssignment(*(new std::string("ASSIGNOP")),*$1,ASSIGNOP,*$3); $$->line=$1->line;}//$$结点type_id空置未用，正好存放运算符
        ...
        | Exp PLUSPLUS   {$$=new NSingleOperator(*(new std::string("PLUSPLUS")),PLUSPLUS,*$1); $$->line=$2;}//这里利用BISON %prec表示和UMINUS同优先级
        | Exp MINUSMINUS  {$$=new NSingleOperator(*(new std::string("MINUSMINUS")),MINUSMINUS,*$1); $$->line=$2;}//这里利用BISON %prec表示和UMINUS同优先级
